    ...
    def minibio(self, extended=True):
        ...
        if self._extended_agent_summary is None and extended:
            ...
            self._extended_agent_summary = openai_utils.LLMRequest(
                system_prompt="""
Вам предоставлена краткая биография агента, а также детальная спецификация его/её характеристик.

Ваша задача - создать короткий абзац (3-4 предложения), который **дополняет** краткую биографию, добавляя детали о:
- личности
- интересах
- мнениях
- навыках
...
Важно:
1. Не повторяйте информацию, уже указанную в краткой биографии
2. Абзац должен быть связным, последовательным и содержательным
3. Вся информация должна основываться на предоставленной спецификации
4. **Не придумывайте** ничего нового
5. Сохраняйте стиль и тон оригинала

Пример правильного выполнения:
"Несмотря на свою сдержанную внешность, Мария обладает острым аналитическим умом. Она увлекается нейробиологией и часто участвует в научных дискуссиях. Её коллеги ценят её за способность находить нестандартные решения сложных задач. В свободное время она предпочитает чтение научной фантастики и шахматы."

Пример неправильного выполнения (повтор информации):
"Мария работает аналитиком в научном центре. Она занимается исследованиями в области нейробиологии..." 
(эта информация уже должна быть в биографии)
                                                """,
                user_prompt=f"""
